...
streaming_process_question(ai_state::AIState; 
    on_meta_usr=noop,  
    on_text=noop,  
    on_meta_ai=noop,
...
function streaming_process_question(state::AIState; on_meta_usr=noop, on_text=noop, on_meta_ai=noop, on_error=nothing, on_done=nothing, on_start=nothing)
    try
        ...
        return curr_conv_msgs(state)[end], shell_scripts, nothing
    catch e
